...
// GOAT's API occasionally blips under the ~15-120-call pulls this dashboard
// does per request — a 5xx or 429 that clears itself a moment later. One
// retry with a short delay absorbs that instead of surfacing the "Couldn't
// load dashboard data" banner to whoever happens to be looking at that
// moment. Deliberately narrow: only network-level failures, 5xx, and 429 —
// a real 4xx (bad param, auth/scope problem) won't change on retry, so
// those still fail immediately with the original error.
